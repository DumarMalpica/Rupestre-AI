...
WARM_BG = colors.HexColor("#fef9f0")
...
def _metrics_row(
    motif_count: int, confidence: float, n_parallels: int, ST: dict
) -> Table:
    col_w = (W - 2 * MARGIN) / 4
...
def _image_cell(img_path: str, label: str, col_w: float, ST: dict) -> list:
...
def _add_page_number(canvas, doc):
    ...
    text = f"{page_num}"
    canvas.drawCentredString(W / 2, 10 * mm, text)
    canvas.restoreState()
...
def generate_pdf(ficha: dict, output_path: str) -> str:
    ...
    if has_parallels:
        alert_text = (
            f"● Paralelos iconográficos regionales encontrados — "
    ...
    if sources:
        ...
        for src in sources:
            ...
            story.append(Paragraph(txt, ST["cite"]))
