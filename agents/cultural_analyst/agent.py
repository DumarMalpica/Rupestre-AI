...
def cultural_analyst_node(state: RupestreState) -> dict:
    ...
    try:
        ...
        cited_sources = [{"title": "Por las tramas de Sutatausa",
                          "author": "Londoño, W.", "year": "2003"}]
        confidence = 0.82
        requires_hitl = confidence < settings.hitl_confidence_threshold

        logger.info(f"Interpretación generada (confianza={confidence}, HITL={requires_hitl})")
        result = {"cultural_interpretation": response,
